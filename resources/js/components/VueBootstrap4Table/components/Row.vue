<template>
    <tr :data-id="rowId" ref="vbt_row" v-bind:style='{ "background": (rowHiglighted) ? highlightRowHoverColor : "" }'
        :class='rowClasses' v-on="rowsSelectable ? { click: ($event) => handleRowSelect($event) } : {}">
        <CheckBox v-if="checkboxRows" :rowsSelectable="rowsSelectable" :row-selected="rowSelected" @add-row="addRow"
...
export default {
    ...
    methods: {
        ...
        cellClasses(column) {
            ...
            let alignments = ["text-justify", "text-right", "text-left", "text-center", 'float-right'];
