        ...
        <tr>
            <th v-show="checkboxRows" class="text-center justify-content-center" @click="selectCheckbox">
                <div class="form-check vbt-select-all-checkbox">
                    ...
                    <va-checkbox v-model="select_all_rows" @change="handleChange($event)"></va-checkbox>
                </div>

            </th>

            <slot name="columns" :columns="columns">
                <th class="table-header-cell" v-for="(column, key, index) in columns" :key="index" v-on="isSortableColumn(column) ? { click: () => $emit('update-sort',column) } : {}" v-bind:class="{'vbt-sort-cursor':isSortableColumn(column)}">
