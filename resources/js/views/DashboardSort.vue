        ...
        <transition-group>
            ...
            <va-table v-if="!fetching" :hover="$root.config.hover" :size="$root.config.size" key="app-table">
                <table>
                    ...
                    <tbody>
                        <tr v-if="!!tableData.rows && tableData.rows.length > 0" v-for="(row, index) in tableData.rows"
                            :key="index">
                            <td>{{ row.id }}</td>
                ...
                <va-pagination :total="tableData.rows.length" :max="tableData.max" @change="pagedItemsChange"
